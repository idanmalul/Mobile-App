...
class GetOfferDetail extends DB {

    function get_offer_detail_by_id() {

            
            if(!empty($_REQUEST['user_id']) && !empty($_REQUEST['sent_primary_id'])){
...
//            $get_story = $this->get_record_where('st', $where);
            ...
            $query = "SELECT c.campaign_name,c.campaign_description,c.reward,s.*,su.id as sent_primary_id,su.campaign_id,su.story_id,su.user_id,su.unique_send_id,su.unique_approval_update_id,TRIM(s.story_description) As story_description,su.upload_status,su.story_accept_status FROM story s INNER JOIN story_user su ON su.story_id=s.id INNER JOIN campaign_story_relationship csr ON csr.campaign_id=su.campaign_id  INNER JOIN campaign c ON c.id=su.campaign_id WHERE (su.user_id='$user_id' AND su.upload_status!=1 AND c.campaign_remaining_count!=0 AND c.campaign_remaining_count>0 AND su.story_accept_status=0 AND (csr.schedule_date_time >= '$current_datetime' OR csr.schedule_date_time IS NULL)) OR (su.user_id='$user_id' AND c.campaign_remaining_count!=0 AND c.campaign_remaining_count>0 AND (csr.schedule_date_time >= '$current_datetime' OR csr.schedule_date_time IS NULL) AND static_campaign_status = 1) AND su.id='$sent_primary_id' GROUP BY su.unique_send_id"; 
...
//                $user_id = $value["user_id"];
                ...
                $query_campaign_schedule = "SELECT csr.*,DAYNAME(DATE(csr.schedule_date_time)) as dayname,DATE_FORMAT(csr.schedule_date_time, '%W %d.%m.%Y | %h:%i %p') as schedule_info FROM campaign c INNER JOIN campaign_story_relationship csr ON csr.campaign_id=c.id WHERE csr.campaign_id='$campaign_id' AND csr.campaign_id='$campaign_id'";
...
$offerDetail = new GetOfferDetail();
$offerDetail->get_offer_detail_by_id();