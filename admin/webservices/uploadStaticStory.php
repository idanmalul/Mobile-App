...
$query_accept = "SELECT s.*,u.username,u.password,su.campaign_id,su.unique_send_id,su.unique_approval_update_id,c.static_campaign_status FROM story s INNER JOIN story_user su ON su.story_id=s.id INNER JOIN users u ON u.user_id=su.user_id INNER JOIN campaign c ON c.id=su.campaign_id WHERE su.user_id='$user_id' AND su.story_id='$story_id' AND su.id='$sent_primary_id' AND su.upload_status!=1 AND c.campaign_remaining_count!=0 AND c.campaign_remaining_count>0";
$get_story = $d->query_result($query_accept);
//echo '<pre>';
//print_r($get_story);
if (!empty($get_story)) {
